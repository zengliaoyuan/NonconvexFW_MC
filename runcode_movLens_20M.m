function [historyIF_0Inf,history_fw_bst,history_afw_bst,scale]=runcode_movLens_20M(sigma,mu,time_limit)
...
dir_exist = exist('ml-20m', 'dir');  % check the existence of the folder ml-20m
dir_exist_7 = dir_exist - 7;
if (dir_exist_7)
    if ~isfile('ml-20m.zip')    % check the existence of the zip file ml-20m.zip
        ...
        fprintf('ml-20m.zip downloaded. \n')
    end 
        unzip('ml-20m.zip', 'ml-20m');
        fprintf('ml-20m.zip unzipped. \n');
end

%% read rating.dat; 
%%%   This data reading part is from read_movielens_10M.m file of InFaceExtendedFW-MatrixCompletion
M = csvread('ml-20m/ml-20m/ratings.csv',1,0,[1,0,20000263,2]);
...
save movielens_20M Omega irow jcol  Xobs_vec;  
...
a = clock;
datname = ['InFaceResults_movielens_20M'  '_'  'timelimit' num2str(time_limit) '_date' date '-' int2str(a(4)) '-' int2str(a(5)) '.mat'];
...
a = clock;
datname = ['FWandAFWResults_movielens_20M_mu'   num2str(mu*100) '_'  'timelimit' num2str(time_limit) '_date' date '-' int2str(a(4)) '-' int2str(a(5)) '.mat'];
...
ylabel('Train Error')
a = clock;
figname = ['movielens_20M-trainingErrs-mu'  num2str(mu*100) '-' date '-' num2str(a(4)) '-'  num2str(a(5))];
...
ylabel('Test Error')
a = clock;
figname = ['movielens_20M-testErrs-mu'  num2str(mu*100) '-' date '-' num2str(a(4)) '-'  num2str(a(5))];
